...
"""MySQL dialect for use with `~lsst.felis.types.SQLTypes`."""

POSTGRESQL = create_mock_engine("postgresql://", executor=None).dialect
"""PostgreSQL dialect for use with `~lsst.felis.types.SQLTypes`."""
...
class SQLTypes(AbstractTypes):
    ...
    type_classes = {
        ...
        "byte": _TF.create(NumericProperties, sql_type=SMALLINT),
        "double": _TF.create(NumericProperties, sql_type=DOUBLE),
        "short": _TF.create(NumericProperties, sql_type=SMALLINT),
        "long": _TF.create(NumericProperties, sql_type=BIGINT),
        "int": _TF.create(NumericProperties, sql_type=INTEGER),
        "float": _TF.create(NumericProperties, sql_type=FLOAT),
        "char": _TF.create(StringProperties, sql_type=CHAR),
