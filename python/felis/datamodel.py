...
ResourceColumnMap = dict[str, ColumnResourceRef | None]
ResourceTableMap = dict[str, ResourceColumnMap]
ResourceMap = dict[str, ResourceTableMap]
...
class Table(BaseObject):
    ...
    def _serialize_columns(self, columns: list[Column]) -> list[dict[str, Any]]:
        ...
        return [
            ...
            for col in columns
            if not getattr(col, "_is_resource_ref", False)
        ]
...
class Schema(BaseObject, Generic[T]):
    ...
    @model_validator(mode="after")
    def _load_resources(self: Schema) -> Schema:
        ...
        for resource_name, resource in self.resources.items():
            uri = resource.uri
            try:
                loaded_schema = Schema.from_uri(uri, context={"id_generation": True})
    ...
    @model_validator(mode="after")
    def _dereference_resource_columns(self: Schema) -> Schema:
                ...
                for resource_name, tables in column_refs.items():
                    ...
                    for table_name, columns in tables.items():
                        try:
                            ...
                            raise ValueError(
                                f"Table '{table_name}' not found in resource '{resource_name}'"
                            ) from e
                        for local_column_name, column_ref in columns.items():
                            ...
                            try:
                                ...
                                raise ValueError(
                                    ...
                                    f"from resource '{resource_name}' and no ref_name provided"
                                )
                            ...
                            logger.debug(
                                ...
                                f"in resource '{resource_name}' into table '{table.name}'"
                            )
        return self
