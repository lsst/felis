...
class TapLoadingVisitor:
    ...
    def visit_schema(self, schema_obj: Schema) -> None:
        ...
        if self.engine is not None:
            ...
            logger.debug(f"Committing TAP schema: {schema_obj.name}")
            logger.debug(f"TAP tables: {len(self.tables)}")
            session.commit()
        else:
    ...
    def visit_table(self, table_obj: Table, schema_obj: Schema) -> tuple:
        ...
        self.visit_primary_key(table_obj.primaryKey, table_obj)
    ...
    def check_column(self, column_obj: dm.Column) -> None:
        _id = column_obj.id
        ...
        if felis_type.is_sized:
            ...
            # length types.
            arraysize = column_obj.votable_arraysize if column_obj.votable_arraysize else column_obj.length
    ...
    def visit_column(self, column_obj: dm.Column, table_obj: Table) -> Tap11Base:
    ...
    def visit_constraint(self, constraint_obj: Constraint, table_obj: Table) -> tuple:
